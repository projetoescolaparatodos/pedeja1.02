...
```javascript
{
  ...
  // NOVO CAMPO: Configuração de taxa dinâmica
  dynamicDeliveryFee: {
    enabled: false,                    // Se está ativo ou não
    tiers: [                           // Faixas de valor
      {
        minValue: 0,                   // Valor mínimo do pedido
        maxValue: 20,                  // Valor máximo do pedido
        customerPays: 5.00,            // Quanto cliente paga
        subsidy: 0                     // Quanto estabelecimento subsidia (calculado)
      },
      {
        minValue: 20,
        maxValue: 50,
        customerPays: 3.00,
        subsidy: 2.00                  // Estabelecimento absorve R$ 2,00
      },
      {
        minValue: 50,
        maxValue: null,                // null = sem limite superior (infinito)
        customerPays: 0,
        subsidy: 5.00                  // Frete totalmente grátis
      }
    ]
  },
  ...
  try {
    ...
    if (enabled) {
      ...
      for (let i = 0; i < tiers.length; i++) {
        ...
        tier.subsidy = parseFloat((deliveryFee - tier.customerPays).toFixed(2));
...
class CartProvider extends ChangeNotifier {
  ...
  double calculateDeliveryFee() {
    ...
    if (_restaurant!.dynamicDeliveryFee?.enabled == true) {
      ...
      print('   Subsídio: R\$ ${matchedTier.subsidy.toStringAsFixed(2)}');
...
### App Flutter
- [ ] Criar models `DynamicDeliveryFeeConfig` e `DeliveryFeeTier`
- [ ] Modificar `Restaurant` model para incluir configuração
- [ ] Atualizar `CartProvider` para calcular taxa dinâmica
- [ ] Modificar UI do carrinho para mostrar taxa atualizada em tempo real
- [ ] Adicionar indicador de "faltam R$ X para frete grátis"
- [ ] Testar cálculo conforme produtos são adicionados/removidos
- [ ] Garantir que valores corretos são enviados para API
